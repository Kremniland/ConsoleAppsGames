...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.delete_img = tk.PhotoImage(file='add.gif')
        btn_delete = tk.Button(toolbar, text='Удаление',
    ...
    def delete_records(self):
        logger.info(self.tree.selection())
        for selection_item in self.tree.selection(): # проходим по выделенным записям и получаем их
                                            # порядковые номера в таблице на экране, а не в базе данных!
            # удаляем записи из таблицы по id = self.tree.set(selection_item, '#1') - первая ячейка из записи в таблице
